#include "krr_agent/action_move_to_drop_location_t1.hpp"
...
namespace krr_agent
{
  ...
  MoveToDropLocation::MoveToDropLocation(const std::string & node_name,
  ...
  MoveToDropLocation::on_configure(const rclcpp_lifecycle::State & previous_state)
  {
    ...
    pos_sub_ = create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>(
     "/amcl_pose",
     10,
     std::bind(&MoveToDropLocation::current_pos_callback, this, _1));
  ...
  {
    send_feedback(0.0, "MoveToDropLocation starting");
    ...
    // --- Get drop_id from PDDL arguments ---
    // move_to_drop_location (?i - item ?l1 ?l2 - location ?r - room)
    // target is ?l2 -> argument index 2
    std::string drop_id = get_arguments()[2];
    RCLCPP_INFO(get_logger(), ">>> drop_id = '%s'", drop_id.c_str());
    ...
    // FIX: Changed from 'item' to 'drop-location' and 'located-target' relation
    request->query =
      "match "
    ...
    // FIX: Using PlanSys2 finish() instead of returning CallbackReturn::FAILURE
    ...
    if (!response->success || response->results.empty()) {
      RCLCPP_ERROR(get_logger(), ">>> No TypeDB results for '%s'", drop_id.c_str());
      finish(false, 0.0, "Drop location not found in TypeDB");
      return plansys2::ActionExecutorClient::on_activate(previous_state);
    }
    ...
    for (auto & result_tree : response->results) {
      for (auto & result : result_tree.results) {
        RCLCPP_INFO(get_logger(), ">>> result.type=%d", result.type);
        if (result.type == ros_typedb_msgs::msg::QueryResult::ATTRIBUTE) {
          const auto & attr = result.attribute;
          RCLCPP_INFO(get_logger(), ">>> attr variable='%s' double_value=%.4f",
            attr.variable_name.c_str(), attr.value.double_value);
    ...
    if (!got_x || !got_y) {
      RCLCPP_ERROR(get_logger(), ">>> Could not parse x/y for '%s'", drop_id.c_str());
      finish(false, 0.0, "Failed to parse coordinates");
      return plansys2::ActionExecutorClient::on_activate(previous_state);
    }

    RCLCPP_INFO(get_logger(), ">>> Found '%s' at (%.2f, %.2f)",
      drop_id.c_str(), obj_x, obj_y);
    ...
    send_goal_options.feedback_callback = [this](
      ...
      NavigationFeedback feedback) {
        send_feedback(
          ...
          "MoveToDropLocation running");
      };

    send_goal_options.result_callback = [this](auto) {
        finish(true, 1.0, "MoveToDropLocation completed");
      };
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<krr_agent::MoveToDropLocation>(
    "move_to_drop_location_t1", 500ms);
